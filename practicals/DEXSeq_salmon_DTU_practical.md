...
```{bash, initial, warning = FALSE, message=FALSE}
# On the terminal run echo every 180 seconds to keep alive
# watch -n180 -x echo "Keep alive"

# REMOVEeditor_options: 
#  chunk_output_type: console
...
```{r, importIsoformExpression, warning = FALSE, message=FALSE, eval = FALSE}
...
```{r, Explore_salmonQuant, warning = FALSE, message=FALSE}
# Explore dataset
...
```{r, importRdata, warning = FALSE, message=TRUE, eval = FALSE}
...
```{r, NO_run, warning = FALSE, message=FALSE, eval = FALSE}
...
```{r, isoformSwitchTestDEXSeq, warning = FALSE, message=FALSE}
...
# Once the isoform switches have been found, the next step is to annotate the isoforms involved in the  
# isoform switches. The first step in such annotation is to obtain Open Reading Frames (ORFs).
...
```{r, analyzeAlternativeSplicing, warning = FALSE, message=FALSE}
### Step (4) Analyze alternative splicing
...
```{r, Visualise, warning = FALSE, message=FALSE}
...
![ BRCA2 ](figures/Rplot_BRCA2.png)  

![ DKK1 ](figures/Rplot_DKK1.png)  

![ HSPA8 ](figures/Rplot_HSPA8.png)  
...
```{r, extractSplicingSummary, warning = FALSE, message=FALSE}
...
![ SplicingSummary ](figures/SplicingSummary.png)  

![ SplicingEnrichment ](figures/SplicingEnrichment.png)  

![ SplicingGenomeWide ](figures/SplicingGenomeWide.png)  

![ SplicingEnrichmentComparison ](figures/SplicingEnrichmentComparison.png)  
...
```{r, consequencesOfInterest, warning=FALSE, message=FALSE,eval=F, echo=T}
